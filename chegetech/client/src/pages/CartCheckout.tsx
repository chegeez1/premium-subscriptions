...
import {
  ...
  AlertCircle, ShoppingCart, Trash2, Package,
} from "lucide-react";
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      if (data.success) {
        ...
        setLocation(`/payment/success?plan=${encodeURIComponent(planNames)}&email=${encodeURIComponent(email)}&cart=1`);
      } else {
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card rounded-2xl p-4 space-y-3">
              {[
                ...
                { icon: Mail, text: "All account credentials delivered to your email instantly" },
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                  ...
                  {configData && !configData.paystackConfigured && (
                    <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/25 text-sm text-red-300">
                      ...
                      <p className="font-semibold">Payments temporarily unavailable. Please try again later.</p>
                    </div>
                  )}

                  <Button
                    type="submit"
                    className="w-full h-12 bg-gradient-to-r from-indigo-600 to-violet-600 border-0 text-white font-bold text-base shadow-xl hover:opacity-90 transition-opacity"
                  ...
                  >
                    {isProcessing || initMutation.isPending || verifyMutation.isPending ? (
                      ...
                      <><CreditCard className="w-4 h-4 mr-2" />Pay KES {totalAmount.toLocaleString()} for All {totalItems} Item{totalItems !== 1 ? "s" : ""}</>
                    )}
                  </Button>
